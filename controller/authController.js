...
exports.protect = async (req, res, next) => {
  ...
  try {
    const decoded = jwt.verify(token, 'secret_text_getting_from_secret_env_or_somewhere_else');
    req.user = decoded.id;
    next();
  } catch (err) {
...
exports.register = async (req, res) => {
  ...
  try {
    ...
    const token = jwt.sign({ id: user.id }, 'secret_text_getting_from_secret_env_or_somewhere_else', {
      expiresIn: 3600
    });
